...
compute_pSST_thresholds <- function(cumulative_n, sigma, alpha){
  thresholds = qnorm(1-alpha/2) * sqrt(cumulative_n) * sigma
  while (estimate_false_detection_rate_bound(cumulative_n, thresholds, sigma) > alpha) {
    thresholds<-thresholds*1.005
  }
  cat("FPR bound =", estimate_false_detection_rate_bound(cumulative_n, thresholds, sigma))
  cat("\nThresholds:", thresholds)
  th<-rep(NA, cumulative_n[length(cumulative_n)])
  th[1:cumulative_n[1]] = thresholds[1]
  for (i in 2:length(cumulative_n)) { 
    th[(cumulative_n[i-1]+1):cumulative_n[i]] = thresholds[i]
  }
  return(th)
}

pSST7 = compute_pSST_thresholds(round((1:7)*(N/7)), sqrt(2)*sigma, alpha)
pSST14 = compute_pSST_thresholds(round((1:14)*(N/14)), sqrt(2)*sigma, alpha)
...
for(r in 1:rep){
  for(e in 1:length(eff)){
    x<-rgamma(N,shape=gamma_k,rate=1/gamma_theta)
    y<-rgamma(N,shape=gamma_k,rate=1/(gamma_theta*(1+eff[e])))
